# This is the final version of my random forest test application.
...
for i in range(0,5):
    ...
    # Set the number of estimators (forest size) the model will use 
    # to make predictions. We set it to 30 based on our test results
    clf = RandomForestClassifier(n_estimators=30)

    # Split the data into training and test sets with a set percentage split
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size = 0.2)

    # Add the test dataset to the random forest classifier
    clf = clf.fit(X_train, Y_train)

    # Stores the prediction based on the test set
    Y_prediction = clf.predict(X_test)

    # Set shuffle_split params for test batches
    cv = ShuffleSplit(n_splits=10, test_size=0.2)

    # Calculate cross value test score using shuffle split params
